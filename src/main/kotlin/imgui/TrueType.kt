...
//            STBTT_memset(pixels, 0, pw*ph) // background of 0 around pixels
    ...
    fun getGlyphInfoT2(info: FontInfo, glyphIndex: Int, i: Vec4i): Int {
        ...
        if (i.x != 0) {
            i.x = if (r != 0) c.min.x else 0
            i.y = if (r != 0) c.min.y else 0
            i.z = if (r != 0) c.max.x else 0
            i.w = if (r != 0) c.max.y else 0
        }
        return if (r != 0) c.numVertices else 0
    }
...
//        if (fdselector == -1) stbtt__new_buf(NULL, 0) TODO useless
    ...
    fun makeGlyphBitmapSubpixel(info: FontInfo, output: ByteBuffer, offsetOutput: Int, out: Vec2i, outStride: Int, scale: Vec2, shift: Vec2,
                                glyph: Int) {
    ...
    fun gGetGlyphShape(info: FontInfo, glyphIndex: Int, vertices: ArrayList<Vertex>) =
            if (info.cff.size == 0) getGlyphShapeTT(info, glyph_index, pvertices)
            else getGlyphShapeT2(info, glyph_index, pvertices)
...
//            vertices = 0
            while (more) {
                ...
                if (comp_num_verts > 0) {
                    // Transform vertices.
                    for (i in 0 until comp_num_verts) {
