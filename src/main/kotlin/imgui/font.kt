...
class FontAtlas {
    ...
    fun addFontDefault(fontCfg: FontConfig) {
...
//
     ...
     *  RGBA32 format is provided for convenience and high compatibility, but note that all RGB pixels are white, so
     *  75% of the memory is wasted.
     *  Pitch = Width * BytesPerPixels  */
    // 1 byte per-pixel
//    fun getTexDataAsAlpha8(unsigned char** out_pixels, int* out_width, int* out_height, int* out_bytes_per_pixel = NULL) {
//
...
//    val decode = Ascii85.decode(defaultCompressedFontDataTTFBase85)
    val decode = decode85(defaultCompressedFontDataTTFBase85)
    println()
}
...
fun decode85(src: String): CharArray {
    ...
    var pSrc = 0
    var dst = CharArray(((defaultCompressedFontDataTTFBase85.length + 4) / 5) * 4)
    var pDst = 0
...
val defaultCompressedFontDataTTFBase85 = "7])#######hV0qs'/###[),##/l:\$#Q6>##5[n42>c-TH`->>#/e>11NNV=Bv(*:.F?uu#(gRU.o0XGH`\$vhLG1hxt9?W`#,5LsCp#-i>.r\$<\$6pD>Lb';9Crc6tgXmKVeU2cD4Eo3R/" +
