...
//                imeWindowHandle = window.hwnd
        ...
        if (installCallbacks) {
            ...
            window.mouseButtonCallbacks["imgui"] = mouseButtonCB
            window.scrollCallbacks["imgui"] = scrollCB
            window.keyCallbacks["imgui"] = keyCB
            window.charCallbacks["imgui"] = charCB
        ...
        if (Platform.get() == Platform.WINDOWS)
            mainViewport.platformHandleRaw = GLFWNativeWin32.glfwGetWin32Window(window.handle.L)
    ...
    private fun updateMousePosAndButtons() {
        ...
        for (viewport in platformIO.viewports) {
            ...
            assert(window.L != NULL)
            val focused = glfwGetWindowAttrib(window.L, GLFW_FOCUSED) != 0
            if (focused) {
                if (io.wantSetMousePos)
                    glfwSetCursorPos(window.L, mousePosBackup.x - viewport.pos.x, mousePosBackup.y - viewport.pos.y)
                else
                    Stack {
                        val mouse = it.nmalloc(Double.BYTES, Double.BYTES * 2)
                        nglfwGetCursorPos(window.L, mouse, mouse + Double.BYTES)
                        ...
                        if (io.configFlags has ConfigFlag.ViewportsEnable) {
                            ...
                            nglfwGetWindowPos(window.L, wnd, wnd + Int.BYTES)
                ...
                for (i in io.mouseDown.indices)
                    io.mouseDown[i] = io.mouseDown[i] || glfwGetMouseButton(window.L, i) != 0
            } else
            ...
            if (GLFW_HAS_GLFW_HOVERED && Platform.get() == Platform.WINDOWS)
                if (glfwGetWindowAttrib(window.L, GLFW_HOVERED) != 0 && viewport.flags hasnt ViewportFlag.NoInputs)
    ...
    private fun updateMouseCursor() {
        ...
        for (n in platformIO.viewports.indices) {
            val window = platformIO.viewports[n].platformHandle as GlfwWindowHandle
            if (imguiCursor == MouseCursor.None || io.mouseDrawCursor)
            // Hide OS mouse cursor if imgui is drawing it or if it wants no cursor
                glfwSetInputMode(window.L, GLFW_CURSOR, GLFW_CURSOR_HIDDEN)
            else {
                // Show OS mouse cursor
                // FIXME-PLATFORM: Unfocused windows seems to fail changing the mouse cursor with GLFW 3.2, but 3.3 works here.
                val cursor = mouseCursors[imguiCursor.i].takeIf { it != NULL } ?: mouseCursors[MouseCursor.Arrow.i]
                glfwSetCursor(window.L, cursor)
                glfwSetInputMode(window.L, GLFW_CURSOR, GLFW_CURSOR_NORMAL)
            }
        }
    }
    ...
    companion object {
        ...
        val mouseButtonCB: MouseButtonCallbackT = { button: Int, action: Int, _: Int ->
            if (action == GLFW_PRESS && button in 0..2)
                mouseJustPressed[button] = true
        }

        val scrollCB: ScrollCallbackT = { offset: Vec2d ->
            io.mouseWheelH += offset.x.f
            io.mouseWheel += offset.y.f
        }

        val keyCB: KeyCallbackT = { key: Int, _: Int, action: Int, _: Int ->
            with(io) {
        ...
        val charCB: CharCallbackT = { c: Int -> if (!imeInProgress) io.addInputCharacter(c.c) }
...
//                    it.platform_CreateVkSurface = ImGui_ImplGlfw_CreateVkSurface
        ...
        fun createWindow(viewport: Viewport) {
            ...
            val shareWindow = window.takeIf { clientApi == GlfwClientApi.OpenGL }?.handle?.L ?: NULL
            ...
            if (Platform.get() == Platform.WINDOWS)
                viewport.platformHandleRaw = GLFWNativeWin32.glfwGetWin32Window(data.window?.handle?.L ?: NULL)
            data.window!!.apply {
                ...
                mouseButtonCallbacks["imgui"] = mouseButtonCB
                scrollCallbacks["imgui"] = scrollCB
                keyCallbacks["imgui"] = keyCB
                charCallbacks["imgui"] = charCB
