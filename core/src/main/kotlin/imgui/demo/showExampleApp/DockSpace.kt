...
object DockSpace {
    ...
    var optFullscreen = true
    var optPadding = false
    ...
    operator fun invoke(pOpen: KMutableProperty0<Boolean>?) {
        ...
        if (optFullscreen) {
            ...
            dockspaceFlags = dockspaceFlags wo DockNodeFlag.PassthruCentralNode
        ...
        if (beginMenuBar()) {
            if (beginMenu("Options")) {
                ...
                menuItem("Fullscreen", "", ::optFullscreen)
                menuItem("Padding", "", ::optPadding)
                separator()

                if (menuItem("Flag: NoSplit", "", dockspaceFlags has DockNodeFlag.NoSplit))
                    dockspaceFlags = dockspaceFlags xor DockNodeFlag.NoSplit
                if (menuItem("Flag: NoResize", "", dockspaceFlags has DockNodeFlag.NoResize))
                    dockspaceFlags = dockspaceFlags xor DockNodeFlag.NoResize
                if (menuItem("Flag: NoDockingInCentralNode", "", dockspaceFlags has DockNodeFlag.NoDockingInCentralNode)
                )
                    dockspaceFlags = dockspaceFlags xor DockNodeFlag.NoDockingInCentralNode
                if (menuItem("Flag: AutoHideTabBar", "", dockspaceFlags has DockNodeFlag.AutoHideTabBar))
                    dockspaceFlags = dockspaceFlags xor DockNodeFlag.AutoHideTabBar
                if (menuItem("Flag: PassthruCentralNode", "", dockspaceFlags has DockNodeFlag.PassthruCentralNode, optFullscreen)                )
                    dockspaceFlags = dockspaceFlags xor DockNodeFlag.PassthruCentralNode
                separator()

                if (menuItem("Close", "", false, pOpen != null))
                    pOpen!!.set(false)
                endMenu()
            }
            helpMarker(
