...
object easyFont {
    ...
    var averageBytesPerChar = 270

    fun drawSegs(x_: Float, y: Float, segs: PtrChar, numSegs: Int, vertical: Int,
                 c: CharArray, vBuf: ByteBuffer, offset_: Int): Int {
        var x = x_
        TODO()
//        var offset = offset_
...
//                    vBuf.putFloat(offset + 8, 0f)
//                    for (k in 0..3)
//                        vBuf[offset + 12 + k] = c[k].b
//                    offset += 16
//                }
//            }
//        }
//        return offset
    ...
    fun print(pos: Vec2 = Vec2(), text: CharSequence, color: CharArray? = null, vertexBuffer: ByteBuffer): Int {
        val vBuf = vertexBuffer
        val startX = pos.x
        var offset = 0

        val c = color ?: CharArray(4) { 255.c } // use structure copying to avoid needing depending on memcpy()

        val pText = PString(text)
        while (pText.isValid && offset < vBuf.cap) {
            if (pText() == '\n' || pText() == '\r') {
                pos.y += 12
                pos.x = startX
            } else {
                val advance = charInfo[pText - 32].advance
                val yCh = pos.y + (advance.i has 16).i
                val hSeg = charInfo[pText - 32].hSeg.i
                val vSeg = charInfo[pText - 32].vSeg.i
                val numH = (charInfo[pText - 32 + 1].hSeg - hSeg).i
                val numV = (charInfo[pText - 32 + 1].vSeg - vSeg).i
                offset = drawSegs(pos.x, yCh, PtrChar(fontInfoHSeg, hSeg), numH, 0, c, vBuf, offset)
                offset = drawSegs(pos.x, yCh, PtrChar(fontInfoVSeg, vSeg), numV, 1, c, vBuf, offset)
                pos.x += advance.i and 15
                pos.x += spacingVal
            }
            pText.next()
        }
    ...
    // @formatter:off
    val charInfo by lazy { charInfoOf(
    ...
    val fontInfoHSeg by lazy { intArrayOf(
        ...
        84,73,57,41,49,25,33,65,81,9,97,1,97,25,33,65,81,57,33,25,41,25).map { it.c }.toCharArray() }

    val fontInfoVSeg by lazy { intArrayOf(
        ...
        20,8,8,28,18,58,89,58,26,21,89,73,89,29,20,8,8,30,7).map { it.c }.toCharArray() }
    // @formatter:on

    fun charInfoOf(vararg ints: Int): Array<InfoStruct> = Array(ints.size / 3) { int ->
        InfoStruct(CharArray(3) { ints[int * 3 + it].c })
    }
}
