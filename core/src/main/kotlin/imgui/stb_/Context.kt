...
constructor(val width: Int,
    ...
    val activeHead: Node?
        get() = activeHeadPtr.value
    var activeHeadPtr: Ptr<Node?> = ptrOf(extra[0])
    var freeHead: Node? = nodes[0]
    ...
    fun skylinePackRectangle(width: Int, height: Int): rectpack.FindResult {
        ...
        if (res.prevLink == null || res.y + height > this.height || freeHead == null)
        ...
        freeHead = node.next!!.value

        // insert the new node into the right starting point, and
        // let 'cur' point to the remaining nodes needing to be
        // stiched back in

        var cur = res.prevLink!!().value!!
        if (cur.x < res.x) {
