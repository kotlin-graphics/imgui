...
fun errorCheckBeginEndCompareStacksSize(window: Window, begin: Boolean) {
    ...
    assert(window.idStack.size == 1) { "PushID/PopID or TreeNode/TreePop Mismatch!" } // Too few or too many PopID()/TreePop();

    // Global stacks
    // For color, style and font stacks there is an incentive to use Push/Begin/Pop/.../End patterns, so we relax our checks a little to allow them.
    run { val n = g.groupStack.size; if (begin) p[i] = n; else assert(p[i] == n) { "BeginGroup/EndGroup Mismatch!" }; i++; } // Too few or too many EndGroup()
    run { val n = g.beginPopupStack.size; if (begin) p[i] = n; else assert(p[i] == n) { "BeginMenu/EndMenu or BeginPopup/EndPopup Mismatch!" }; i++; } // Too few or too many EndMenu()/EndPopup()
    run { val n = g.colorModifiers.size; if (begin) p[i] = n; else assert(p[i] >= n) { "PushStyleColor/PopStyleColor Mismatch!" }; i++; } // Too few or too many PopStyleColor()
    run { val n = g.styleModifiers.size; if (begin) p[i] = n; else assert(p[i] >= n) { "PushStyleVar/PopStyleVar Mismatch!" }; i++; } // Too few or too many PopStyleVar()
    run { val n = g.fontStack.size; if (begin) p[i] = n; else assert(p[i] >= n) { "PushFont/PopFont Mismatch!" }; i++; } // Too few or too many PopFont()
