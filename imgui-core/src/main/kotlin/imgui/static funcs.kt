...
fun navScoreItem(result: NavMoveResult, cand: Rect): Boolean {
    ...
    val dbY = navScoreItemDistInterval(imgui.imgui.lerp(cand.min.y, cand.max.y, 0.2f), imgui.imgui.lerp(cand.min.y, cand.max.y, 0.8f),
            imgui.imgui.lerp(curr.min.y, curr.max.y, 0.2f), imgui.imgui.lerp(curr.min.y, curr.max.y, 0.8f))
    if (dbY != 0f && dbX != 0f)
    ...
    if (IMGUI_DEBUG_NAV_SCORING)
        if (ImGui.isMouseHoveringRect(cand.min, cand.max)) {
            ...
            getForegroundDrawList(window).apply {
                ...
                addRectFilled(cand.max - Vec2(4), cand.max + ImGui.calcTextSize(buf) + Vec2(4), COL32(40, 0, 0, 150))
