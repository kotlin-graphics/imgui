    ...
    operator fun invoke(open: KMutableProperty0<Boolean>) {
        ...
        if (beginTabBar("##TabBar")) {
            ...
            if (beginTabItem("BG/FG draw lists")) {
                ...
                if (drawBg)
                    ImGui.backgroundDrawList.addCircle(windowCenter, windowSize.x * 0.6f, COL32(255, 0, 0, 200), 48, 10f + 4)
                if (drawFg)
                    ImGui.foregroundDrawList.addCircle(windowCenter, windowSize.y * 0.6f, COL32(0, 255, 0, 200), 48, 10f)
                endTabItem()
            }
            endTabBar()
        }
