...
//        println("" + g.openPopupStack.size +", "+currentStackSize)
    ...
    fun renderTextEllipsis(drawList: DrawList, posMin: Vec2, posMax: Vec2, clipMaxX: Float, ellipsisMaxX: Float,
        ...
        if (textSize.x > posMax.x - posMin.x) {
            ...
            if (textEndEllipsis[0] != textEndFull) {
                ...
                var c = 0
                val extraSpacing = 0f
                var textEndEllipsisPrev = textEndEllipsis[0]
                TODO()
...
//                    IM_ASSERT(apply_new_text_length <= buf_size);
    ...
    fun shadeVertsLinearColorGradientKeepAlpha(drawList: DrawList, vertStart: Int, vertEnd: Int, gradientP0: Vec2,
        ...
        for (i in vertStart until vertEnd) {
            ...
            val pos = Vec2(drawList.vtxBuffer.data, offset)
    ...
    fun shadeVertsLinearUV(drawList: DrawList, vertStart: Int, vertEnd: Int, a: Vec2, b: Vec2, uvA: Vec2, uvB: Vec2, clamp: Boolean) {
        ...
        if (clamp) {
            ...
            val max = uvA max uvB
            for (i in vertStart until vertEnd) {
                val vertexPos = Vec2(drawList.vtxBuffer.data, i * DrawVert.size)
