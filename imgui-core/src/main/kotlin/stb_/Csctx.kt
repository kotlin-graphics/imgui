...
class Csctx(val bounds: Boolean) {
    ...
    lateinit var vertices: Array<tt.Vertex>
    var numVertices = 0
    ...
    fun v(type: tt.V, x: Int, y: Int, cx: Int, cy: Int, cx1: Int, cy1: Int) {
        if (bounds) {
            trackVertex(x, y)
            if (type == tt.V.cubic) {
                trackVertex(cx, cy)
    ...
    fun closeShape() {
        if (firstX != x || firstY != y)
            v(tt.V.line, firstX.i, firstY.i, 0, 0, 0, 0)
    }
    ...
    fun rMoveTo(dx: Float, dy: Float) {
        ...
        y += dy
        firstY = y
        v(tt.V.move, x.i, y.i, 0, 0, 0, 0)
    }

    fun rLineTo(dx: Float, dy: Float) {
        x += dx
        y += dy
        v(tt.V.line, x.i, y.i, 0, 0, 0, 0)
    }
    ...
    fun rcCurveTo(dx1: Float, dy1: Float, dx2: Float, dy2: Float, dx3: Float, dy3: Float) {
        ...
        x = cx2 + dx3
        y = cy2 + dy3
        v(tt.V.cubic, x.i, y.i, cx1.i, cy1.i, cx2.i, cy2.i)
    }
}